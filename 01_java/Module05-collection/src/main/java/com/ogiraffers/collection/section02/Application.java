package com.ogiraffers.collection.section02;

/*
* Map 기초
* - hashMap은 키-값 쌍으로 데이터를 관리하는 컬렉션이다.
* - 각 키는 유일해야 하며, 중복된 키를 사용할 수 없다. 동일한 키에 값을 변경하려면 기존 값이 새 값으로 대체된다.
* - 실생활 비유 : 사전에 단어(키)와 그에 대한 정의(값)을 저장하는 느낌.
* = 데이터의 빠른 검색 및 접근이 가능하며, 키를 통해 직접적으로 값을 조회할 수 있다.
*
* hash란??
* - hash는 데이틀 고정된 크기의 값으로 변환하는 과정을 의미한다. 일반적으로 해시 함수(hash funtion)을 통해 이루어진다.
* - 해시 함수는 입력값(예 : 키)를 받아 해시 코드(정수)를 생성하며, 이 해시 코드는 hashMap의 내부 배열에서 인덱스로 사용된다.
...
*   - 요수의 순서를 보장하려면 linkedHashMap을 사용할 수 있다.
* 4. 성능 :
...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        // map의 크기 확인
        System.out.println("HashMapSize : " + bookMap.size());

        System.out.println("책의 목록 하나씩 확인 : ");
        for (Map.Entry<Integer, String> entry : bookMap.entrySet()) {
            System.out.println("key : " +  entry.getKey() + " value : " + entry.getValue());
        }

        if (bookMap.containsKey(3)){
            System.out.println("키 3에 해당하는 값" + bookMap.get(3));
        } else {
