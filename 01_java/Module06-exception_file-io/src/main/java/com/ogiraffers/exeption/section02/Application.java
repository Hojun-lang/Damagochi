package com.ogiraffers.exeption.section02;

/*
* 예외 처리
*   - 예외(Exception)은 프로그램 실행 중 예상치 못한 문제로 인해 정상적인 흐름이 중단되는 상황을 의미한다.
*   - 예를 들어, 사용자가 0으로 나누기를 시도하거나, 파일을 열 때 파일이 존재하지 않는 경우 등이 있다.
*   - java에서는 try-catch블록을 통해 이러한 예외를 처리하며, finally블록으로 필수 정리 작업을 보장한다.
*   - 사용자 정의 예외는특정 상황에서 개발자가 직접 예외를 만들어 사용하는 것으로 코드의 의도를 명확하게 전달한다.
*
*  예외 처리가 필요한 이유
*   - 예외가 발생하면 프로그램이 비정삭적으로 종료된다.
...
//            System.out.println("결과 : " + result);
            ...
            System.out.println("Exception 모두 잡음");
        } finally {
            sc.close();
